...
MODULAR_GROUPS = {
    ...
    "Insole Room": [
        ...
        "left navicular sweet spot", "right navicular sweet spot",
        "right custom hole and plug", "left custom hole and plug",
        "right all mets", "left all mets",
        "additional info cut outs",
        "vinyl black", "microfibre brown", "multi ld eva 1.5mm", "black ld eva 1.5mm",
        "black ld eva 3mm", "spenco 1.5mm", "spenco 3mm", "on steam",
        "no top cover", "no lining", "additional info top cover",
        "soft poron 1.6mm", "soft poron 3mm", "soft poron 6mm",
        "medium poron 1.6mm", "medium poron 3mm", "medium poron 6mm",
        "firm memory foam poron 1.6mm", "firm memory foam poron 3mm", "firm memory foam poron 6mm"
    ],
}
...
class PdfButtonHandler:
    ...
    def get_price_codes_from_content(self, content, logic_content):
        ...
        system_prompt = f"Use the following logic to generate price codes:\n\n{logic_content}\n\nThe 'Passed code' section contains codes that have already been generated and should be included in the final output.\n\nDoubling rule: Structured pair selection (e.g. 'afo pair: selected', 'insole pair: selected', 'pair: selected') AND a numeric 'order quantity' greater than 1 have ALREADY been applied to the passed codes by the system - do NOT double or multiply the quantities because of them. Only look for a free-text instruction written by the clinician in the cradle details or additional information sections (e.g. 'make two', 'two shoes', 'make a pair', 'duplicate', 'x2', 'double it'). If such a free-text instruction is present AND no structured pair field is selected AND order quantity is not already greater than 1, double all quantities in the passed codes (e.g. 'B55B' becomes 'B55B x2'). If a structured pair field IS selected, or order quantity is already greater than 1, do NOT apply any further doubling, even if the free text also mentions a pair, x2, or 'double it'. If the free text clearly requests a specific larger quantity than what is already in the passed codes (e.g. 'make 4' when the codes are only x2), apply that exact multiple; if the requested quantity is unclear, do not change the quantities. Otherwise, repeat the passed codes exactly as they are.\n\nFirst, write your full working out, explaining step-by-step and why. Then, always write **Final Codes:** followed by the final codes. Formatting requirement: each code MUST be on its own separate line. NEVER put more than one code on a line and NEVER separate codes with commas. For example:\nP15 x2\nD8U\nD1C\nDo not include any additional text or summary after the final codes. Ensure the **Final Codes:** section is always present, even if no changes are made."
